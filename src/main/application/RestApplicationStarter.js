...
function RestApplicationStarter(){
  ...
  this.run = async (callerDirectoryLocation) => {
    ...
    if(environment !== 'production'){
      var headAnnotations = ["Config","Route", "Middleware", "ServerInitializer"];
  ...
  this.startServer = async (dependencies) => {
    ...
    for(let dependency of dependencies){
      ...
      if(typeof dependency.meta.arguments.name !== 'undefined'){
        instanceId = dependency.meta.arguments.name;
      }else{
        instanceId = path.basename(dependency.meta.location).replace(".js","");
      }
  ...
  getInstanceId = (dependency) => {
    if(typeof dependency.meta.arguments.name !== 'undefined'){
      ...
      return path.basename(dependency.meta.location).replace(".js","");
    }
  }
}
